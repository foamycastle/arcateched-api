...
export default class deleteAttachment extends MachineData {

    opName: string

    constructor() {
        super();
        this.opName = 'deleteAttachment'
    ...
    queryPreparation(): (request: extendedRequest, response: extendedResponse, next: NextFunction) => void {
        return (request: extendedRequest, response: extendedResponse, next: NextFunction) => {
            ...
            this.preparedQuery = preparedQuery(validata.id,validata.attachmentId)
